            ...
            <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
                ...
                <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                    ...
                    <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                        ...
                        <li class="breadcrumb-item text-muted">
                            <a href="{{route('admin.roles')}}" class="text-muted text-hover-primary">{{__('master.roles')}}</a>
                        </li>
                        <li class="breadcrumb-item">
                            <span class="bullet bg-gray-400 w-5px h-2px"></span>
                        </li>
                        <li class="breadcrumb-item text-muted">{{__('master.edit role')}}</li>
                    </ul>
                </div>
            </div>
        </div>
                    ...
                    <div >
                        <div class="modal-content">
                            <div class="modal-header">
                                <h2 class="fw-bold">{{__('master.edit role')}}</h2>
                            </div>
                            @if ($errors->any())
                            ...
                            <div >
                                <form action="{{route('admin.roles.update',$role->id)}}" method="POST">
                                    ...
                                    <div class="d-flex flex-column scroll-y me-n7 pe-7"   data-kt-scroll-activate="{default: false, lg: true}" data-kt-scroll-max-height="auto" data-kt-scroll-dependencies="#kt_modal_add_role_header" data-kt-scroll-wrappers="#kt_modal_add_role_scroll" data-kt-scroll-offset="300px">
                                        <div class="fv-row mb-10">
                                            <label class="fs-5 fw-bold form-label mb-2">
                                                <span class="required">{{__('master.role name')}}</span>
                                            </label>
                                            <input class="form-control form-control-solid" placeholder="{{__('master.enter a role name')}}" name="name" value="{{$role->name}}" />
                                        </div>
                                        <div class="fv-row">
                                            <label class="fs-5 fw-bold form-label mb-2">{{__('master.role permissions')}}</label>
                                                                ...
                                                                <td>
                                                                    <div class="d-flex">
                                                                        <label class="form-check form-check-sm form-check-custom form-check-solid me-5 me-lg-20">
                                                                            <input class="form-check-input" type="checkbox" value="{{ $name }}" name="permissions[]" {{ in_array($name, $role->permissions)? 'checked' : '' }}/>
                                                                            <span class="form-check-label">{{__('master.check')}}</span>
                                                                        </label>
                                    ...
                                    <div class="card-footer d-flex justify-content-end py-6 px-9">
                                        ...
                                        <button type="submit" class="btn btn-primary" >{{__('master.update role')}}</button>
                                    </div>
                                </form>
                            </div>
                        </div>
